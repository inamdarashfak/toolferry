...
export type HealthHighlightSeverity = 'normal' | 'attention' | 'critical'

export function getHealthHighlightSx(
  theme: Theme,
  severity: HealthHighlightSeverity
) {
  if (severity === 'normal') {
    return {
  ...
  return {
    ...
    border: `1px solid ${alpha(
      tone,
      theme.palette.mode === 'dark' ? 0.5 : 0.22
    )}`,
    backgroundColor: alpha(tone, theme.palette.mode === 'dark' ? 0.18 : 0.08),
  }
}
